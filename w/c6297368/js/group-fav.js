window.TR21_BASE = "/w/c6297368";
window.TR21_VERSION = "20260925-001";
...
(function () {
  ...
  function add(u, t, s, p) {
    const list = load();
    if (list.some(f => f.u === u)) return;
    const rec = {u: u, t: t, s: s, ts: Date.now()};
    /* p — название страницы, на которой лежит подраздел. Заголовок раздела сам
       по себе безымянный («Ответ курса» встречается в трёхстах разборах), так
       что в списке избранного показываем «Страница › Раздел». У самой страницы
       поля нет — формат расширяется, старые записи читаются как есть. */
    if (p) rec.p = p;
    list.push(rec);
    save(list);
  }
  ...
  function toggle(u, t, s, p) { has(u) ? del(u) : add(u, t, s, p); return has(u); }
  ...
  async function loadOrder() {
    ...
    try {
      const arr = await fetch('/w/c6297368/js/fav-order.json').then(r => r.json());
  ...
  /* Текст заголовка без служебных значков. Их в конце накапливается сразу
     три: «§» — постоянная ссылка от mkdocs, «☆»/«★» — наша звезда, «✎» —
     карандаш комментария. Звёзды расставляются по очереди (сначала h1, потом
     подзаголовки), поэтому к моменту чтения h1 в нём уже лежат наши же кнопки,
     и в избранное уезжало «Выбор партнёра§☆✎». */
  const TITLE_TAIL = /[\u00a7\u2606\u2605\u270e\s]+$/;
  function trim(text) { return String(text || '').replace(TITLE_TAIL, '').trim(); }
  function plainTitle(el) { return el ? trim(el.textContent) : ''; }

  /* ── звёздочка ──────────────────────────────────────────────────────── */
  function starEl(u, t, s, p) {
    ...
    b.addEventListener('click', e => { e.preventDefault(); toggle(u, t, s, p); sync(); });
  ...
  function glossaryStars() {
    ...
    document.querySelectorAll('.md-content a[id^="t-"]').forEach(a => {
      ...
      if (!strong) return;
      const u = '/теория/глоссарий/#' + a.id;   /* ключ избранного — без базы */
      strong.appendChild(starEl(u, plainTitle(strong), 'глоссарий'));
      strong.appendChild(noteEl(u, p));
    });
  }

  /* Звёзды у подзаголовков страницы — то же, что у терминов глоссария, только
     ключ строится из адреса страницы и id заголовка. Слаг id делает mkdocs, а
     тот же слаг считает генератор для js/fav-order.json, поэтому подраздел
     встаёт в списке сразу за своей страницей.

     Заголовки берём из содержимого, а не из правого «Содержания»: на мобильном
     его нет вовсе, а заголовок есть всегда. Уровень h4 и глубже не трогаем —
     это уже не подраздел, а перечисление внутри него. */
  function headingStars() {
    const s = sectionOf(relPath());
    if (!s) return;
    const h1 = document.querySelector('.md-content h1');
    /* название страницы запоминаем в момент добавления: страницы зашифрованы,
       и потом, не открыв страницу, его не достать (та же причина, что у t) */
    const par = plainTitle(h1);
    document.querySelectorAll('.md-content h2, .md-content h3').forEach(h => {
      if (!h.id || h.querySelector('.fav-star')) return;
      const t = plainTitle(h);
      if (!t) return;
      const u = relPath() + '#' + h.id;   /* ключ избранного — без базы */
      h.appendChild(starEl(u, t, s, par));
      h.appendChild(noteEl(u, h));
    });
  }
  ...
  async function renderInto(box, list, sig) {
    ...
    shown.forEach(f => {
      html += '<li class="fav-item">'
        ...
        /* в избранном хранится адрес БЕЗ базы — ключ должен быть одинаков во всех
         сборках; базу добавляем только при отрисовке ссылки */
      /* значки чистим и здесь: записи, добавленные до этой правки, уже лежат
         в localStorage со «§☆✎» на хвосте, и переписать их нам нечем */
      +   '<a href="' + BASE + esc(f.u) + '">'
        +     (f.p ? '<span class="fav-list__par">' + esc(trim(f.p))
                     + '<span class="fav-crumb">\u203a</span></span>' : '')
        +     esc(trim(f.t)) + '</a>'
  ...
  function unlocked() {
    /* Признак — ОТСУТСТВИЕ формы входа. Два предыдущих варианта были неверны:
       encryptcontent_done плагин ставит в true и когда расшифровать нечем
       («попытка закончена», а не «вошёл»), а контейнер #mkdocs-decrypted-content
       он показывает сразу — САМА ФОРМА ЛЕЖИТ ВНУТРИ НЕГО, иначе её не было бы
       видно. При верном пароле плагин переписывает контейнер содержимым, и
       форма исчезает. Страница без шифрования формы не имеет — там открыто. */
    return !document.getElementById('mkdocs-decrypt-form');
  }
  ...
  function all() {
    [pageStar, headingStars, glossaryStars, navMark, logoutButton, favsSidebar, renderFavs]
      .forEach(fn => {
