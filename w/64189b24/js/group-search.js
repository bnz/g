...
(function () {
  ...
  async function loadDocs() {
    ...
    docs = [];
    try {
      const clear = await fetch('/w/64189b24/search/search_index.json').then(r => r.json());
      docs.push(...(clear.docs || []));
    } catch (e) {}
    try {
      const enc = await fetch('/w/64189b24/search/encrypted_search_index.json').then(r => r.json());
  ...
  async function run(q, out, cnt) {
    ...
    out.innerHTML = scored.slice(0, 40).map(pair => {
      ...
      return '<article><div class="gs-meta">' + LABEL[d._s] + path + '</div>'
        + '<h4><a href="/w/64189b24/' + d.location + '">' + esc(d.title || d.location) + '</a></h4>'
  ...
  function personsSidebar() {
    ...
    roster.querySelectorAll('a').forEach(a => {
      ...
      ppl += '<li class="md-nav__item roster-li"><a class="md-nav__link'
        ...
        + '" href="/w/64189b24/люди/' + slug + '/"><span class="md-ellipsis">'
  ...
  async function fetchMenu() {
    ...
    let html;
    try { html = await fetch('/w/64189b24/меню/').then(r => r.text()); } catch (e) { return null; }
